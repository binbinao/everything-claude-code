...
| 🎣 **Hooks** | 47 个脚本 / 7 事件 | 智能触发代码检查、格式化等自动化操作 |
| 📏 **Rules** | 21 个语言包 | TypeScript/Python/Go/Rust/Ruby/Swift/Vue/React 等 |
...
Hooks 能在特定事件发生时自动执行操作，覆盖 7 类事件：

- **SessionStart** - 会话开始时加载上下文、检测项目状态
- **PreToolUse** - 工具使用前拦截（pre:bash、pre:write、pre:edit 等）
- **PostToolUse** - 工具使用后格式化、检查、记录
- **PreCompact** - 上下文压缩前持久化会话状态
- **PostToolUseFailure** - 工具失败时记录
- **Stop** - 每次响应后做格式检查
- **SessionEnd** - 会话结束时收尾、归档

干跑模式（`--dry-run`）允许在不真正执行命令的情况下预览 hook 行为（v2.0.0）。
